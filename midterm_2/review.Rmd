...
In the following block, we're going to take 100 samples of size $n=2, 5, 30$. Ignore the code. Just take a look at the output.
...
##### Sampling with n=2
```{r}
sample_means <- do.call(rbind, lapply(1:100, function(x) sample_n_salaries(2)))
sample_means %>% summarize(n_5_mean=mean(mean_salary))
```

```{r}
ggplot(sample_means, aes(x=mean_salary)) + geom_histogram(binwidth=0.2)
```

##### Sampling with n=5
```{r}
sample_means <- do.call(rbind, lapply(1:100, function(x) sample_n_salaries(5)))
sample_means %>% summarize(n_5_mean=mean(mean_salary))
```
...
```{r}
sample_means <- do.call(rbind, lapply(1:100, function(x) sample_n_salaries(30)))
sample_means %>% summarize(n_30_mean=mean(mean_salary))
```

```{r}
ggplot(sample_means, aes(x=mean_salary)) + geom_histogram(binwidth=0.1)
```
