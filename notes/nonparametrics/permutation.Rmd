...
Typically, in order to test something like the above, we'd want to use a two sample t-test for the comparison of two means or a Mann-Whitney test. But our small sample size makes both of these tests work less than optimally.

Recall, for a t-test we would have:

$$
T= \frac{\bar{x_1}-\bar{x_2}}{\hat{SE}(\bar{x_1}-\bar{x_2})}
$$

And for Mann-Whitney, we would use values dependent on $n_1$ and $n_2$. But again, none of these estimates are quite robust to sample size.
