...
**DISCLAIMER:** You do not need to know any of the code you're going to see on this worksheet. You just need to understand the output.
...
4. Surely, we always want our sample to representative of our population. We do this by taking _________ samples. Also, just to be clear, just because the samples are said to be just that, they are not taken haphazardly.  

5. When we want to make a histogram in the library ______________, we use the function ________________.  

6. In `dplyr`, we use the following symbol %>% called the _____________________ to send our data into functions.  

## Short Answer.
1. Because assumptions, such as large sample size, may be difficult to satisfy, we have alternative methods. We can evade the large sample size requirement when testing hypotheses by using a ____________ test instead of formal testing procedures.

2. When we construct a 95% confidence interval using the _____________ method, we have to find the 2.5th percentile and the 97.5 percentile of our estimated "sampling" distribution to capture ____% of the data within these bounds.

## Santa's Little Statistician.
<i>Directions: It is your job to talk to the elf.</i>

Welcome to Santa's Workshop! We really needed some help so we decided to invite you, a cheery statistician, to the North Pole! This year, we're working on sending good folk as much presents as possible! That being said, Santa himself said we could only use 15 total presents to make a confidence interval about the weights of the presents. Why weight, you say? Well, Santa's sleigh can only carry up to 1000 lbs! What? Did you think the sleigh was magic? 
...
given these data. I chose these presents randomly!
...
**We sample from our sample because we want to estimate our ____________ distribution of the statistic we are interested in, which is what confidence intervals are normally based off of.**  

What was the median of our original present weight dataset?    

**Our sample's median is _______** <i>(numeric value)</i>.

Gotcha! Also, here are the 1000 medians you asked for earlier in histogram format. I also plotted the 5th percentile and the 95th percentile. You said you could make a confidence interval out of it. So what is the interval?  
...
And here's a preview (the first five columns only) of what the computer is giving us after running the code you wrote to switch up the shipping labels. 
...
I understood how you switched up the labels, but I actually don't understand how you got to this next part with all these sample means. What exactly did you do and why?  
